...
def find_img(article_title, upplaga_nmr, base_url):
    ...
    new_img_title = utils.remove_åäö(utils.make_image_id(article_title))
...
def generate_static_section(title, content, image_src):
    ...
    final_article = f"""
<article class="Artikel" id="{title.replace(" ", "_")}">
    {image_context}
    <h2>{title}</h2>
...
def get_base(path):
    ...
    for replacment in replacment_for_all:
        content = content.replace(replacment, replacment_for_all[replacment])
...
formated_staff_file_content = utils.make_regex_list_to_dict(utils.file_parser(staff_file_content))
...
"""
    ...
    for upplaga in reversed(content_reader.read_articles()):
        ...
        for article_bundle in upplaga["Content"]:
            article = article_bundle[0]
            if article: # somethimes article is empty, this prevents that
                article_img_src = find_img(utils.remove_html_elements(str(article["Rubrik"])), upplaga_number, "https://ostraloken.se/a/images/") # get the url to the right image
                whole_content_articles += generate_lone_article("SHOULD_NOT_REDIRECT", article_img_src, str(article["Rubrik"]), str(article["Artikel"]), str(article["Texttyp"]), str(article["Skribent"]), 0, upplaga_number)
...
"""
    ...
    for content in content_reader.read_txt("static/articles.txt"):
        ...
        # generate article
        generated_section = generate_static_section(content["Rubrik"], content["Artikel"], content["Bild_källa"])
        replacment_dictionary[f"[+{replacment_name}+]"] = generated_section # like ex "[+test+]"
        
        # generate without image
        generated_section = generate_static_section(content["Rubrik"], content["Artikel"], "")
        replacment_dictionary[f"[+no_img_{replacment_name}+]"] = generated_section # like ex "[+no_img_test+]"
        
        # add just the article
        replacment_dictionary[f"[+{replacment_name}_article+]"] = content["Artikel"] # like ex "[+test_article+]"
...
"""
    ...
    replacment_dictionary["[+latest_title+]"] = utils.remove_html_elements(most_recent_story_list["Rubrik"]).replace('"', "&quot;")
...
def generate_webbsite(webb_path, template_path):
    ...
    for file_dir in Path(template_path).iterdir():
        if Path(file_dir).is_file(): # If it is not a folder
            ...
            generate_site(file_dir, full_destination_dir, replacment_for_all, file_dir.stem)
