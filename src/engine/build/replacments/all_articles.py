...
for upplaga in reversed(content_reader.read_articles()):
    ...
    for article_bundle in upplaga["Content"]:
        ...
        if article: # somethimes article is empty, this prevents that
            ...
            whole_content_articles += build_articles.generate_lone_article("SHOULD_NOT_REDIRECT", article_img_src, str(article["Rubrik"]), str(article["Artikel"]), str(article["Texttyp"]), str(article["Skribent"]), -1, upplaga_number)
