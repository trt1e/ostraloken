...
normal_story_path = work_path(r"\ostraloken\content\normal_storys_and_other")

short_story_path = work_path(r"\ostraloken\content\short_storys.txt")
short_story_selection = {"Title": {"start": "### ", "end": " ##"}, "Article": {"start": "+++ ", "end": " ++"}}

hear_me_outs_path = work_path(r"\ostraloken\content\hear_me_outs.txt")
hear_me_outs_selection = {"Hear_me_out": {"start": "### ", "end": " ##"}, "Description": {"start": "+++ ", "end": " ++"}}

staff_info_path = work_path(r"\ostraloken\content\static\staff.txt")
staff_info_selection = {"Name": {"start": "### ", "end": " ##"}, "Description": {"start": "+++ ", "end": " ++"}, "Title": {"start": '""" ', "end": ' ""'}, "Email": {"start": "@@@ ", "end": " @@"}, "Image_src": {"start": "§§§ ", "end": " §§"}}

static_articles_path = work_path(r"\ostraloken\content\static\articles.txt")
static_articles_selection = {"Title": {"start": "### ", "end": " ##"}, "Article": {"start": "+++ ", "end": " ++"}, "Image_src": {"start": "§§§ ", "end": " §§"}}

external_links_content_path = work_path(r"\ostraloken\content\static\external_links.txt")
...
def copy_over_images(gen_type):
    instagram_article_image_template_path = work_path(r"\ostraloken\instagram_images\templates\Normal_article.png")
    instagram_upplaga_image_template_1_path = work_path(r"\ostraloken\instagram_images\templates\Upplaga_1.png")
    instagram_upplaga_image_template_2_path = work_path(r"\ostraloken\instagram_images\templates\Upplaga_2.png")
    
    generated_images_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\a\images" + "\\")
    generated_upplaga_images_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\pdfer\pdf_images" + "\\")
    
    generated_instagram_images_path = work_path(r"\ostraloken\instagram_images\final_images" + "\\")
    
    all_articles = read_normal_storys()
    ...
    for upplaga in all_articles:
        ...
        for article in upplaga["Content"]:
            if article: # somethimes article is empty, this prevents that
                ...
                old_img_title = make_image_id(article_title)
                new_img_title = remove_åäö(make_image_id(article_title)) + ".webp"
                new_img_title_insta = f"{image_number + 1}-" + remove_åäö(make_image_id(article_title))[4:].split(".")[0] + ".png"
                ...
                if old_img_path_with_extention != "NO_IMG": # Create insta image
                    ...
                    if gen_type != "new" or os.path.isfile(instagram_image_destination_dir) is False: # either gen_typ isn't new, or if it is, we still let it pass if there is no file
                        ...
                        if create_image_switch: 
                            ...
                            upplaga_qr.add_data(f"https://ostraloken.se/a/{make_article_id(article_title, upplaga_number)}")
                            ...
                            if image_number == 1: # if its the first article, so it only does this once per upplaga
                                upplaga_first_page_dir = generated_upplaga_images_path + f"Upplaga_{upplaga_number}" + handel_path_slash("\\") + "page_1.webp"
                                    ...
                                    if upplaga_number == new_upplaga["Upplaga"]:
                                        for new_article_number, new_article in enumerate(new_upplaga["Content"]):
                                            if new_article: # somethimes article is empty, this prevents that
                                                ...
                                                new_article_title = "● " + str(remove_html_elements(new_article["Title"]))
...
def copy_over_pdfs(gen_type):
    pdf_start_path = work_path(r"\ostraloken\content\pdfs" + "\\")
    pdf_file_end_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\pdfer\pdf_files" + "\\")
    pdf_images_end_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\pdfer\pdf_images" + "\\")
    ...
    pdf_js_program_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\pdfer\js\PDF_reader.js")
...
def generate_site(template_path, generated_path, dictionary_of_replacment, file_type): # the sites that dont realy need to be generated    
    ...
    # add header
    header_read = get_base(r"\ostraloken\webb\ostraloken.se\templates\base\header.html")
    ...
    # add footer
    footer_read = get_base(r"\ostraloken\webb\ostraloken.se\templates\base\footer.html")
    final_file = final_file.replace("[+footer+]", footer_read) # add footer
    
    # add general scripts
    scripts_read = get_base(r"\ostraloken\webb\ostraloken.se\templates\base\general_scripts.html")
...
def generate_all_articles():
    article_template_path = work_path(r"\ostraloken\webb\ostraloken.se\templates\a\articles_pages.html")
    generated_articles_path = work_path(r"\ostraloken\webb\ostraloken.se\webbpage\a" + "\\")
    
    list_of_articles_with_similer_type = generate_preview_article("./a/", "Similar")
    all_short_storys = read_txt(short_story_path, short_story_selection)
...
def get_nav_element(img_switch, highlight_switch):
    ...
    for links_bundle in  read_txt(external_links_content_path, external_links_content_selection):
...
"""
    ...
    for short_story_bundle in reversed(read_txt(short_story_path, short_story_selection)):
...
"""
    ...
    for hear_me_out_bundle in reversed(read_txt(hear_me_outs_path, hear_me_outs_selection)):
...
"""
    ...
    for static_bundle in reversed(read_txt(static_articles_path, static_articles_selection)):
    ...
    generated_sections = ""
    staff_list = ""
    for staff_bundle in read_txt(staff_info_path, staff_info_selection):
...
def generate_all_normal_pages(): # go throught every file in templates
    template_base_paths = [
        work_path(r"\ostraloken\webb\nyhetsflode.ostraloken.se\templates" + "\\"),
        work_path(r"\ostraloken\webb\ostraloken.se\templates" + "\\")
    ]
...
--------------------------------------------------""")
                ...
                print("Restarting...")
                subprocess.run(f'python -u "{work_path(r"\ostraloken\main.py")}"')
                break
...
<@&{discord_role_taged_in_reminders}>, skriv skriv skriv!!!
...
Tack på förhand :heart: :heart: """
                    send_discord_message(bot_message)
            elif answer == "bot send":
                bot_input = input("Message: ")
                if bot_input != "":
                    send_discord_message(bot_input)
...
GÖR MER MODULÄR!!!
Gör så set dir fungerar och så den fungerar bättre på flera platser oavsätt var man kör den!!!
