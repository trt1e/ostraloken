...
def inspect_normal_storys(): # looks throught all files to se if something is wrong but doesnt change nothing
    print("↓ ARTICLES ↓")
    upplaga_list = os.listdir(read_content.normal_story_path) # list all folders in dir
    for upplaga in upplaga_list: # go thrpguth every folder to get all the upplagor
        ...
        file_list = os.listdir(read_content.normal_story_path / upplaga)
        for file in file_list: # Go througth every file in the list
            if file[:4] != "IMG-":
                # check if there are img files that do not start with IMG-
                # OLD: if file[-3:] in main.img_extentions or file[-4:] in main.img_extentions:
                if file.endswith(tuple(main.img_extentions)):
                    print(f'WARNING: {file} is image but does not start with "IMG-" as it should')
                    printed_something = True
                else:
                    try:
                        whole_text = base_commands.try_opening(read_content.normal_story_path / upplaga / file, "tr")
                        ...
                        if file != "upplaga_info.txt":
                            ...
                            if whole_text.find("### ") != -1 and whole_text.find(" ##") != -1:
                                ...
                                titles_list.append(base_commands.strip_string(base_commands.remove_html_elements(title), 100))
                                
                                if str("_".join(file.split(" ")[1:])).split(".")[0] != base_commands.strip_string(base_commands.remove_html_elements(title), 100):
                                    print(f'WARNING: {file} (fixed: {str("_".join(file.split(" ")[1:])).split(".")[0]}) does not have same name as title: {title} (fixed: {base_commands.strip_string(base_commands.remove_html_elements(title), 100)})')
                                    printed_something = True
...
def inspect_short_storys():
    whole_text = base_commands.try_opening(read_content.short_story_path, "tr") # read it
    print("\n↓ NOTISER ↓")
    if re.search(r"[“”]", whole_text): # if “ or ” in file, should be "
    ...
    for number_of_articles in range(whole_text.count("### ")): # repeat for how many hear me outs there are in the txt
        ...
        title = base_commands.find_between(whole_text, "### ", " ##", last_final_pos)
        if "++" in title:
        ...
        article = base_commands.find_between(whole_text, "+++ ", " ++", last_final_pos)
        if "++" in article:
...
def inspect_hear_me_outs():
    whole_text = base_commands.try_opening(read_content.hear_me_outs_path, "tr") # read it
    print("\n↓ HEAR ME OUTS ↓")
    if re.search(r"[“”]", whole_text): # if “ or ” in file, should be "
    ...
    for number_of_articles in range(whole_text.count("### ")): # repeat for how many hear me outs there are in the txt
        # find where diffrent parts are in the document
        hear_me_out = base_commands.find_between(whole_text, "### ", " ##", last_final_pos)
        if "++" in hear_me_out:
        ...
        desc = base_commands.find_between(whole_text, "+++ ", " ++", last_final_pos)
        if "++" in desc:
...
def fix_citationmarks():
        ...
        for file in file_list: # Go througth every file in the list
            if file[:4] != "IMG-":
                ...
                if re.search(r"[“”]", whole_text): # if “ or ” in file, should be "
                    ...
                    open_content = open(read_content.normal_story_path / upplaga / file, "w", encoding="utf-8")
    ...
    if re.search(r"[“”]", whole_text): # if “ or ” in file, should be "
        new_text = whole_text.replace("“", '"')
        new_text = new_text.replace("”", '"')
        open_content = open(read_content.short_story_path, "w", encoding="utf-8")
        open_content.write(new_text) # write to it
        open_content.close()
    ...
    if re.search(r"[“”]", whole_text): # if “ or ” in file, should be "
        new_text = whole_text.replace("“", '"')
        new_text = new_text.replace("”", '"')
        open_content = open(read_content.hear_me_outs_path, "w", encoding="utf-8")
        open_content.write(new_text) # write to it
        open_content.close()
    ...
    for upplaga in upplaga_list: # go thrpguth every folder to get all the upplagor
        ...
        for file_number, file in enumerate(file_list, 1): # Go througth every file in the list and extract the content
            if file != "upplaga_info.txt" and file[:4] != "IMG-":
                ...
                title = base_commands.find_between(whole_text, "### ", " ##", 0)
                basic_title = base_commands.remove_html_elements(title)
                
                new_file_name = str(file_number) + " " + base_commands.strip_string(basic_title, 100) + ".txt"
                
                os.rename((read_content.normal_story_path / upplaga / file), (read_content.normal_story_path / upplaga / new_file_name))
